...
const RiwayatPelanggaranRedirect: React.FC = () => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchAndRedirect = async () => {
      try {
        ...
        if (match) {
          navigate(`/data-siswa/${match.id_siswa}?tab=riwayat`, { replace: true });
        } else {
          setError("Data riwayat pelanggaran Anda tidak ditemukan di sistem.");
        }
      } catch (err) {
        console.error(err);
        setError("Gagal memuat data riwayat pelanggaran.");
      }
    };
...
export default RiwayatPelanggaranRedirect;
