  ...
  useEffect(() => {
    const fetchRombelReport = async () => {
      try {
        ...
        const res = await axios.get(`http://localhost:3000/api/print-report?id_rombel=${id}`, {
          headers: {
        ...
        setError("Gagal memuat rekapitulasi rombel. Pastikan Anda memiliki akses.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
        ...
        <p className="mt-4 text-gray-600 dark:text-gray-400 font-medium">Mempersiapkan rekapitulasi rombel...</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-6 text-center">
        <div className="bg-red-50 text-red-700 px-6 py-4 rounded-xl border border-red-200 max-w-md shadow-sm">
          ...
          <p className="text-sm">{error || "Data rekapitulasi tidak dapat dimuat."}</p>
        </div>
        <button
          onClick={() => navigate(-1)}
      ...
      <div className="no-print mb-8 flex items-center justify-between bg-gray-50 border border-gray-100 rounded-xl p-4 shadow-sm max-w-5xl mx-auto">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-medium text-gray-500">
            Preview Rekapitulasi Poin Rombel
          </span>
        </div>
        <button
      ...
      <div className="max-w-6xl mx-auto border border-gray-200 print:border-none p-10 print:p-0 rounded-2xl shadow-md print:shadow-none bg-white">
        ...
        <div className="text-center mb-8 border-b border-gray-300 pb-4">
          ...
          <p className="text-xs text-gray-500 mt-1">
            Sistem Informasi Ketertiban dan Kendali Disiplin Sekolah
          </p>
        </div>

        {/* Rombel Details Info Block */}
        {rombelInfo && (
          <div className="border border-gray-300 rounded-xl p-5 mb-8 bg-gray-50/50 flex flex-wrap justify-between gap-4 text-xs">
            <div className="space-y-2">
              <div>
                <span className="font-semibold text-gray-500 w-32 inline-block">Kelas / Rombel</span>
                <span className="mr-2">:</span>
                <span className="text-gray-900 font-bold">{rombelInfo.rombel}</span>
              </div>
              <div>
                <span className="font-semibold text-gray-500 w-32 inline-block">Jurusan</span>
                <span className="mr-2">:</span>
            ...
            <div className="space-y-2">
              <div>
                <span className="font-semibold text-gray-500 w-32 inline-block">Semester</span>
                <span className="mr-2">:</span>
                <span className="text-gray-900">{rombelInfo.semester || "Semua Semester"}</span>
              </div>
              <div>
                <span className="font-semibold text-gray-500 w-32 inline-block">Wali Kelas</span>
                <span className="mr-2">:</span>
                <span className="text-gray-900 font-semibold">{rombelInfo.walikelas}</span>
              </div>
            </div>
          </div>
        )}
          ...
          <table className="w-full text-left border-collapse border border-gray-300 text-sm">
            <thead>
              <tr className="bg-gray-100 text-gray-700 font-bold border-b border-gray-300">
                ...
                <th className="p-3 w-48 text-center">ACTION / SALDO POIN</th>
              </tr>
            </thead>
            <tbody>
              {students.map((student, idx) => (
                <tr key={student.id_siswa} className="border-b border-gray-300 hover:bg-gray-50/40">
                  <td className="p-3 border-r border-gray-300 text-center font-medium">{idx + 1}</td>
                  <td className="p-3 border-r border-gray-300 font-semibold text-gray-800 uppercase">{student.nama_siswa}</td>
                  <td className="p-3 border-r border-gray-300 font-mono text-xs">{student.nisn || "-"}</td>
                  <td className="p-3 text-center">
                    <button
                      onClick={() => navigate(`/print-laporan/${student.id_siswa}`)}
                    ...
                    {/* Plain text version shown only during printing */}
        ...
        {rombelInfo && (
          <div className="flex justify-end mt-16 text-center text-sm print:break-inside-avoid">
            <div className="mr-8">
              <p className="text-gray-500 mb-20">Mengetahui,<br />Wali Kelas</p>
