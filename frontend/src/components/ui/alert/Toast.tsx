import React, { useEffect, useState } from "react";
import { createPortal } from "react-dom";

export type ToastVariant = "success" | "error" | "warning" | "info";

interface ToastProps {
  show: boolean;
  variant?: ToastVariant;
  message: string;
  duration?: number; // ms before auto-dismiss (default 3000)
  onClose: () => void;
}

const ICONS: Record<ToastVariant, React.ReactNode> = {
  success: (
    <svg className="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
      />
    </svg>
  ),
  error: (
    <svg className="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"
      />
    </svg>
  ),
  warning: (
    <svg className="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
      />
    </svg>
  ),
  info: (
    <svg className="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"
      />
    </svg>
  ),
};

const STYLES: Record<ToastVariant, string> = {
  success:
    "bg-green-50 border-green-400 text-green-800 dark:bg-green-900/40 dark:border-green-500 dark:text-green-300",
  error:
    "bg-red-50 border-red-400 text-red-800 dark:bg-red-900/40 dark:border-red-500 dark:text-red-300",
  warning:
    "bg-yellow-50 border-yellow-400 text-yellow-800 dark:bg-yellow-900/40 dark:border-yellow-500 dark:text-yellow-300",
  info:
    "bg-blue-50 border-blue-400 text-blue-800 dark:bg-blue-900/40 dark:border-blue-500 dark:text-blue-300",
};

const Toast: React.FC<ToastProps> = ({
  show,
  variant = "info",
  message,
  duration = 3000,
  onClose,
}) => {
  const [visible, setVisible] = useState(false);
  const [mounted, setMounted] = useState(false);

  useEffect(() => {
    if (show) {
      setMounted(true);
      // small delay so the enter transition fires after mount
      const enterTimer = setTimeout(() => setVisible(true), 10);
      const exitTimer = setTimeout(() => {
        setVisible(false);
        setTimeout(() => {
          setMounted(false);
          onClose();
        }, 300);
      }, duration);
      return () => {
        clearTimeout(enterTimer);
        clearTimeout(exitTimer);
      };
    } else {
      setVisible(false);
      const t = setTimeout(() => setMounted(false), 300);
      return () => clearTimeout(t);
    }
  }, [show, duration, onClose]);

  if (!mounted) return null;

  const handleManualClose = () => {
    setVisible(false);
    setTimeout(() => {
      setMounted(false);
      onClose();
    }, 300);
  };

  // Render via portal so it always mounts on document.body,
  // escaping any stacking-context or transform parent (e.g. AppLayout)
  return createPortal(
    <div
      style={{ position: "fixed", top: "1.25rem", right: "1.25rem", zIndex: 99999 }}
      className={`flex items-center gap-3 px-4 py-3 rounded-lg border shadow-xl
        max-w-sm w-full transition-all duration-300
        ${STYLES[variant]}
        ${visible ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-3"}
      `}
      role="alert"
    >
      {/* Icon */}
      <span className="flex-shrink-0">{ICONS[variant]}</span>

      {/* Message */}
      <p className="flex-1 text-sm font-medium">{message}</p>

      {/* Close button */}
      <button
        onClick={handleManualClose}
        className="flex-shrink-0 opacity-60 hover:opacity-100 transition-opacity"
        aria-label="Tutup"
      >
        <svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
          />
        </svg>
      </button>
    </div>,
    document.body
  );
};

export default Toast;
