...
module.exports = {
    ...
    async add(req, res) {
        ...
        try {
            ...
            if (checkJabatan.rows.length === 0) {
                return res.status(404).json({ error: "Data Guru tidak ditemukan di master PTK" });
            }

            if (checkJabatan.rows[0].id_jabatan !== '21904' && checkJabatan.rows[0].id_jabatan !== 21904) {
                return res.status(403).json({ 
                    error: "Akses ditolak. PTK yang dipilih bukan merupakan Guru BK (Jabatan Tidak Sesuai)" 
                });
            }
            ...
            if (error.code === '23505' || error.message.includes('unique_rombel_semester')) {
            try {
                ...
                return res.status(409).json({
                    error: "Data Rombel sudah dimiliki oleh Guru lain pada semester ini"
                });
            }
    ...
    async update(req,res){
        ...
        try {
            ...
            if (checkJabatan.rows[0].id_jabatan !== '21904' && checkJabatan.rows[0].id_jabatan !== 21904) {
                return res.status(403).json({ error: "PTK baru yang dipilih bukan Guru BK" });
            }
